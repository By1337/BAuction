...
public class BuyItemProcess {
    ...
    public BuyItemProcess(@NotNull MemorySellItem buyingItem, @NotNull MemoryUser buyer) {
        this.buyingItem = buyingItem;
        this.buyer = buyer;
    }

    public BuyItemProcess(@NotNull MemorySellItem buyingItem, @NotNull MemoryUser buyer, @Nullable CyclicList<Category> categories, @Nullable CyclicList<Sorting> sortings, int currentPage) {
        this.buyingItem = buyingItem;
        this.buyer = buyer;
...
//        MemoryUser user = Main.getStorage().update(buyer);
//
//        if (user == null) {
...
//                        if (seller.isOnline()){
//                            Main.getMessage().sendMsg(seller.getPlayer(), "&aИгрок %s купил у вас %s за %s!", bukkitPlayer.getName(), item.getMaterial(), NumberUtil.format(item.getPrice()));
//                        }

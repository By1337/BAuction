...
public record Sorting(SortingType type, String value, String selectedName,
    ...
    public Comparator<PluginSellItem> getComparator(){
        if (type == Sorting.SortingType.COMPARE_MAX) {
            ...
            return switch (value){
                case "{price}" -> Comparator.comparingDouble(PluginSellItem::getPrice);
                case "{price_for_one}" -> Comparator.comparingDouble(PluginSellItem::getPriceForOne);
                case "{sale_time}" ->
                        Comparator.comparingDouble((PluginSellItem item) -> (double) item.getTimeListedForSale() / 1000);
                default -> throw new IllegalArgumentException("unknown sorting type: " + this);
            };
        }
    }
